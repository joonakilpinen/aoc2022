...
func ParseInstructions(input string) []Instruction {
	...
	for _, line := range strings.Split(input, "\n") {
		if len(line) > 0 {
			...
			case "addx":
				addition, err := strconv.Atoi(split[1])
				if err != nil {
					log.Fatal(err)
				}
				instructions = append(instructions, Instruction{addx, addition})
			}
		}
	}
	return instructions
...
func ProcessInstructions(instructions []Instruction) map[Cycle]int {
	cycleScores := map[Cycle]int{}
	var currentCycle Cycle = 1
	x := 1
	for _, instruction := range instructions {
		switch instruction.cycle {
		case noop:
			cycleScores[currentCycle] = x
			currentCycle += noop
		case addx:
			for i := 0; i < int(addx); i++ {
				cycleScores[currentCycle] = x
				currentCycle++
			}
			x += instruction.addition
		}
	}
	return cycleScores
}

func sumCycleScores(cycleScores map[Cycle]int, cycles ...Cycle) int {
	sum := 0
	for _, cycle := range cycles {
		sum += cycleScores[cycle] * int(cycle)
	}
	return sum
}
...
func (Solver) Solve(input string) string {
	...
	cycleScores := ProcessInstructions(instructions)
	sum := sumCycleScores(cycleScores, 20, 60, 100, 140, 180, 220)
