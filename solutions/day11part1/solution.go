...
func (m *Monkey) getInspectedItemAndThrowTarget() (int, int) {
	m.operate()
...
func processMonkeys(monkeys []Monkey) {
	for i := 0; i < len(monkeys); i++ {
		for monkeys[i].hasItems() {
			item, target := monkeys[i].getInspectedItemAndThrowTarget()
...
//    one at a time and in the order listed. Monkey 0 goes first, then monkey
//    1, and so on until each monkey has had one turn. The process of each
