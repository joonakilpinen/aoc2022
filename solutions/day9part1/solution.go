...
func ParseDirections(input string) []Direction {
	...
	for _, line := range strings.Split(input, "\n") {
		if len(line) > 0 {
			...
			directions = append(directions, Direction{move, steps})
		}
	}
	return directions
}
...
func MoveTail(head Coordinates, tail Coordinates) Coordinates {
	if isAdjacent(head, tail) {
		return Coordinates{tail.X, tail.Y}
	}
	xDifference := head.X - tail.X
	if xDifference < 0 {
		xDifference = -1
	}
	if xDifference > 0 {
	...
	return Coordinates{tail.X + xDifference, tail.Y + yDifference}
}

func MoveHead(head Coordinates, direction Direction) Coordinates {
	switch direction.Move {
	case UP:
		head = Coordinates{head.X, head.Y + 1}
	case DOWN:
		head = Coordinates{head.X, head.Y - 1}
	case LEFT:
		head = Coordinates{head.X - 1, head.Y}
	case RIGHT:
		head = Coordinates{head.X + 1, head.Y}
	}
	return head
...
func WalkTheLine(directions []Direction, length int) map[Coordinates]bool {
	var knots []Coordinates
	for i := 0; i < length; i++ {
		knots = append(knots, Coordinates{0, 0})
	}
	tailVisited := map[Coordinates]bool{}
	tailVisited[knots[len(knots)-1]] = true
...
func (Solver) Solve(input string) string {
	...
	tailVisited := WalkTheLine(directions, 2)
