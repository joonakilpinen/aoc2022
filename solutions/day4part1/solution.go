...
func assignmentInAssignment(first assignment, second assignment) bool {
	return first.start <= second.start && first.end >= second.end
}

func duplicateRanges(pair assignmentPair) bool {
	return assignmentInAssignment(pair.first, pair.second) || assignmentInAssignment(pair.second, pair.first)
}

func duplicateRangeCount(assignmentPairs []assignmentPair) int {
	count := 0
	for _, pair := range assignmentPairs {
		if duplicateRanges(pair) {
			count++
		}
	}
	return count
...
func (Solver) Solve(input string) string {
	...
	return strconv.Itoa(duplicateRangeCount(assignmentPairs))
}
