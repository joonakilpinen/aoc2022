...
func isBorderingTree(matrix [][]int, x int, y int) bool {
	if x == 0 {
		return true
	}
	if y == 0 {
...
func isNotXYBlocked(matrix [][]int, x int, y int) bool {
	if !isXBlocked(matrix, x, y) {
		return true
	}
	if !isYBlocked(matrix, x, y) {
		return true
	}
	return false
}

func isXBlocked(matrix [][]int, x int, y int) bool {
	height := matrix[x][y]
	blockedDirections := 0
	for i := y - 1; i >= 0; i-- {
...
func isYBlocked(matrix [][]int, x int, y int) bool {
	height := matrix[x][y]
	blockedDirections := 0
	for i := x - 1; i >= 0; i-- {
...
func isNotBlocked(matrix [][]int, x int, y int) bool {
	var predicates []func(matrix [][]int, x int, y int) bool
	predicates = append(predicates, isBorderingTree)
	predicates = append(predicates, isNotXYBlocked)
	for _, predicate := range predicates {
		if predicate(matrix, x, y) {
			return true
		}
	}
	return false
...
func countNotBlocked(matrix [][]int) int {
	sum := 0
	for x := 0; x < len(matrix); x++ {
		for y := 0; y < len(matrix[x]); y++ {
			if isNotBlocked(matrix, x, y) {
				sum++
			}
		}
	}
