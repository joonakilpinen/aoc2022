...
func draw(scores map[day10part1.Cycle]int) string {
	result := ""
	var keys []int
	...
	sort.Ints(keys)
	for _, cycle := range keys {
		x := scores[day10part1.Cycle(cycle)]
...
func (Solver) Solve(input string) string {
	instructions := day10part1.ParseInstructions(input)
	cycleScores := day10part1.ProcessInstructions(instructions)
	drawing := draw(cycleScores)
	return drawing
}
